...
class flatten(object):
    ...
    def forward(self, feat):
        ...
        #############################################################################
        featShape = feat.shape[0]
        featShapeCol = feat.shape[1:]
        
        output = feat.reshape(featShape, np.prod(featShapeCol))
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = feat
...
class fc(object):
    ...
    def backward(self, dprev):
        ...
        self.grads[self.b_name] = np.dot(np.ones((1, feat.shape[0])), dprev).flatten()
...
class gelu(object):
    ...
    def forward(self, feat):
        ...
        # Store the results in the variable output provided above.                  #
        #############################################################################
        cdf = 0.5 * (1.0 + np.tanh((np.sqrt(2/np.pi) * (feat + 0.044715 * np.power(feat, 3)))))
        output = feat * cdf
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
    ...
    def backward(self, dprev):
        ...
        #############################################################################
        epsilon = 1e-8
        y_plus = self.forward(feat + epsilon)
        y_minus = self.forward(feat - epsilon)
...
class dropout(object):
    ...
    def forward(self, feat, is_training=True, seed=None):
        ...
        if is_training and self.keep_prob != 1 and self.keep_prob > 0:
            kept = self.rng.binomial(1, self.keep_prob, size = feat.shape)
            inv = (1.0 / self.keep_prob)
            output = feat * kept * inv
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.kept = kept
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        logprobs = -np.log(logit[range(featShape), label])
        if self.size_average:
            loss = np.sum(logprobs)/featShape
        else:
    ...
    def backward(self):
        ...
        dlogit = logit
        if self.size_average:
            dlogit = dlogit / self.label.shape[0]
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.logit = None
...
def softmax(feat):
    ...
    #############################################################################
    e_x = np.exp(feat - np.max(feat, axis = -1, keepdims=True))
    scores = e_x / np.sum(e_x, axis=-1).reshape(-1,1)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    return scores
